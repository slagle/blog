...
$mysitefeed = get_bloginfo('rss2_url');
 ...
 if (get_option('greeting') || get_option('welcomemessage')) {
  if (get_option('greeting')) {
    echo "<strong>" . get_option('greeting') . "</strong>";
    }
  if (get_option('welcomemessage')) {
    echo "<p>" . get_option('welcomemessage') . "</p>";
    }
  } else {
  echo "<h3>Hey. Welcome to $mysitename!</h3><p>Thanks for dropping by! Feel free to join the discussion by leaving comments, and stay updated by subscribing to the <a href='$mysitefeed'>RSS feed</a>. See ya!</p>";
}
